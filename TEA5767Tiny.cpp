...
TEA5767Tiny::TEA5767Tiny() {
	TinyWireM.begin();
	HILO = 1;
}

TEA5767Tiny::TEA5767Tiny(double initial_freq) {
	TinyWireM.begin();
	HILO = 1;
...
int TEA5767Tiny::hilo_optimal (unsigned long freq) {
...
void TEA5767Tiny::set_frequency (int hilo, double freq) {
...
void TEA5767Tiny::set_frequency (double freq) {
...
int TEA5767Tiny::read_status (unsigned char *buf) {
	memset (buf, 0, 5);
	TinyWireM.requestFrom (0x60, 5);    //reading TEA5767
...
void TEA5767Tiny::search_up (unsigned char *buf) {
  	unsigned div;
  	double freq_av;
...
void TEA5767Tiny::search_down (unsigned char *buf)
{
  	unsigned div;
  	double freq_av;
...
int TEA5767Tiny::process_search (unsigned char *buf, int search_dir)
{
  	if (ready (buf) == 1) {
      	if (bl_reached (buf) == 1) {
